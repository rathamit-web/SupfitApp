...
BEGIN
  IF NOT EXISTS (
    SELECT 1
    FROM pg_policies
    WHERE polname = 'active_hours_select_own'
      AND polrelid = 'public.active_hours'::regclass
  ) THEN
  ...
  IF NOT EXISTS (
    SELECT 1
    FROM pg_policies
    WHERE polname = 'active_hours_insert_own'
      AND polrelid = 'public.active_hours'::regclass
  ) THEN
  ...
  IF NOT EXISTS (
    SELECT 1
    FROM pg_policies
    WHERE polname = 'active_hours_update_own'
      AND polrelid = 'public.active_hours'::regclass
  ) THEN
  ...
  IF NOT EXISTS (
    SELECT 1
    FROM pg_policies
    WHERE polname = 'active_hours_select_coach'
      AND polrelid = 'public.active_hours'::regclass
  ) THEN
...
BEGIN
  IF NOT EXISTS (
    SELECT 1
    FROM pg_policies
    WHERE polname = 'source_connections_select_own'
      AND polrelid = 'public.source_connections'::regclass
  ) THEN
...
BEGIN
  IF NOT EXISTS (
    SELECT 1
    FROM pg_policies
    WHERE polname = 'mcp_envelopes_select_own'
      AND polrelid = 'public.mcp_envelopes'::regclass
  ) THEN
...
BEGIN
  IF NOT EXISTS (
    SELECT 1
    FROM pg_policies
    WHERE polname = 'model_access_logs_select_own'
      AND polrelid = 'public.model_access_logs'::regclass
  ) THEN
